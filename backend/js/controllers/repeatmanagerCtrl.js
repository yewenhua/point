...
.controller('RepeatmanagerCtrl', ['$scope', 'CommonFunction', 'DataLoad', 'toaster', 'divpage', '$modal', '$stateParams', function($scope, CommonFunction, DataLoad, toaster, divpage, $modal, $stateParams) {
	$scope.haveData = true;
	$scope.loading = true;
	$scope.query = '';
	$scope.page = 1;
	...
	$scope.$watch('is_delete', function(new_val, old_val){
	    if(new_val != old_val){
	    	$scope.page = 1;
	        $scope.getAllData();
	    }
	...
	$scope.getAllData = function() {
		  $scope.loading = true;
		  var promise = DataLoad.getRepeatPageData({
			  searchkey: $scope.query,
			  is_delete: $scope.is_delete ? 1 : 0,
		  ...
		  return promise.then(function (returnData) {
				if (returnData && returnData.code == 0) {
					...
					$scope.dataList = [];
					$scope.haveData = false;
					toaster.pop('error', '复投管理', returnData.message);
				}
				$scope.loading = false;
	...
	$scope.deleteBatch = function(){
	   if(confirm("确定要删除吗?") == true){
		  if($scope.submit == false){
			  ...
			  var promise = DataLoad.deleteRepeatBatch({
			  ...
			  return promise.then(function (returnData) {
					...
					if (returnData && returnData.code == 0) {
						toaster.pop('success', '复投管理', returnData.message);
						location.reload();
					} 
					else {
						$scope.submit = false;
