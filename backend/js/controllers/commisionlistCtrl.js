...
.controller('CommisionlistCtrl', ['$scope', 'CommonFunction', 'DataLoad', 'toaster', 'divpage', '$modal', '$stateParams', function($scope, CommonFunction, DataLoad, toaster, divpage, $modal, $stateParams) {
	$scope.haveData = true;
	$scope.loading = true;
	$scope.query = '';
	$scope.page = 1;
	...
	$scope.getAllData = function() {
		  $scope.loading = true;
		  var promise = DataLoad.getCommisionPageData({
		  ...
		  return promise.then(function (returnData) {
				if (returnData && returnData.code == 0) {
					$scope.commision = returnData.data.commision;
					$scope.order_money = returnData.data.order_money;
					$scope.pay_money = returnData.data.pay_money;
					...
					$scope.dataList = [];
					$scope.haveData = false;
					toaster.pop('error', '佣金记录', returnData.message);
				}
				$scope.loading = false;
	...
	$scope.deleteBatch = function(){
	   if(confirm("确定要删除吗?") == true){
		  if($scope.submit == false){
			  ...
			  $scope.all_delete_disabled = true;
			  var promise = DataLoad.deleteCommisionBatch({
			  ...
			  return promise.then(function (returnData) {
					...
					if (returnData && returnData.code == 0) {
						toaster.pop('success', '佣金记录', returnData.message);
						$scope.page = 1;
						$scope.getAllData();
					} 
					else {
