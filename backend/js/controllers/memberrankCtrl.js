...
.controller('MemberrankCtrl', ['$scope', 'CommonFunction', 'DataLoad', 'toaster', 'divpage', '$modal', '$stateParams', '$window', function($scope, CommonFunction, DataLoad, toaster, divpage, $modal, $stateParams, $window) {
	$scope.haveData = true;
	$scope.loading = true;
	$scope.query = '';
	$scope.page = 1;
	...
	$scope.thList = [
	    {
	    	name : '消费积分', sort : 'fa-sort-desc', selected: true
	    },
	    {
	    	name : '购物劵', sort : 'fa-sort', selected: false
	    },
	    {
	    	name : '可用积分', sort : 'fa-sort', selected: false
	    },
	    {
	    	name : '待用积分', sort : 'fa-sort', selected: false
	    },
	    {
	    	name : '分享积分', sort : 'fa-sort', selected: false
	    },
	    {
	    	name : '佣金', sort : 'fa-sort', selected: false
	    }
	];
	...
	$scope.changeSort = function(item){
		...
		else if(item.name == '购物劵'){
		    $scope.sort.word = 'exchange_point';
		}
		else if(item.name == '可用积分'){
		    $scope.sort.word = 'useable_point';
		}
		else if(item.name == '待用积分'){
		    $scope.sort.word = 'wait_point';
		}
		else if(item.name == '分享积分'){
		    $scope.sort.word = 'share_point';
		}
		else if(item.name == '佣金'){
		    $scope.sort.word = 'commision';
		}
		else{
			$scope.sort.word = 'consume_point';
		}
	...
	$scope.getAllData = function() {
		  $scope.loading = true;
		  var promise = DataLoad.getMemberStatisticPageData({
		  ...
		  return promise.then(function (returnData) {
				if (returnData && returnData.code == 0) {
					...
					$scope.total.total_useable_point = returnData.total.total_useable_point;
					$scope.total.total_wait_point = returnData.total.total_wait_point;
					$scope.total.total_share_point = returnData.total.total_share_point;
					$scope.total.total_commision = returnData.total.total_commision;
					$scope.total.total_member = returnData.total.total_member;
					$scope.total.total_vip = returnData.total.total_vip;
					...
					$scope.dataList = [];
					$scope.haveData = false;
					toaster.pop('error', '会员排行', returnData.message);
				}
				$scope.loading = false;
		  }, function () {
			  $scope.dataList = [];
