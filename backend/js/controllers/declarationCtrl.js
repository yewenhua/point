...
.controller('DeclarationCtrl', ['$scope', 'CommonFunction', 'DataLoad', 'toaster', 'divpage', '$modal', '$stateParams', function($scope, CommonFunction, DataLoad, toaster, divpage, $modal, $stateParams) {
	$scope.haveData = true;
	$scope.loading = true;
	$scope.query = '';
	$scope.page = 1;
	...
	if(!angular.isUndefined($stateParams.userid) && !angular.isUndefined($stateParams.type)){
	...
	$scope.getAllData = function() {
		  $scope.loading = true;
		  var promise = DataLoad.getDeclarationPageData({
		  ...
		  return promise.then(function (returnData) {
				if (returnData && returnData.code == 0) {
					$scope.recharge = returnData.data.recharge;
					$scope.point = returnData.data.point;
					...
					$scope.dataList = [];
					$scope.haveData = false;
					toaster.pop('error', '报单记录', returnData.message);
				}
				$scope.loading = false;
	...
	$scope.deleteBatch = function(){
	   if(confirm("确定要删除吗?") == true){
		  if($scope.submit == false){
			  ...
			  var promise = DataLoad.deleteDeclarationBatch({
			  ...
			  return promise.then(function (returnData) {
					...
					if (returnData && returnData.code == 0) {
						toaster.pop('success', '报单记录', returnData.message);
						location.reload();
					} 
					else {
						$scope.submit = false;
