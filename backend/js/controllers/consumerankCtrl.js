...
.controller('ConsumerankCtrl', ['$scope', 'CommonFunction', 'DataLoad', 'toaster', 'divpage', '$modal', '$stateParams', '$window', function($scope, CommonFunction, DataLoad, toaster, divpage, $modal, $stateParams, $window) {
	$scope.haveData = true;
	$scope.loading = true;
	$scope.query = '';
	$scope.page = 1;
	...
	$scope.getAllData = function() {
		  $scope.loading = true;
		  var promise = DataLoad.getConsumeStatisticPageData({
		  ...
		  return promise.then(function (returnData) {
				if (returnData && returnData.code == 0) {
					...
					$scope.dataList = [];
					$scope.haveData = false;
					toaster.pop('error', '消费排行', returnData.message);
				}
				$scope.loading = false;
		  }, function () {
			  $scope.dataList = [];
	...
	$scope.detail = function(item) {
	    ...
	    var end = $scope.dateSelectRange.end.format('YYYY-MM-DD');
  	    //$window.location.href = "/admin#/app/orderlist?key=" + item.mobile + "&status=3&start=" + start + "&end=" + end;
    }
}]);
