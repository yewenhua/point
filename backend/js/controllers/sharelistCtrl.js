...
.controller('SharelistCtrl', ['$scope', 'CommonFunction', 'DataLoad', 'toaster', 'divpage', '$modal', '$stateParams', function($scope, CommonFunction, DataLoad, toaster, divpage, $modal, $stateParams) {
	$scope.haveData = true;
	$scope.loading = true;
	$scope.query = '';
	$scope.page = 1;
	...
	$scope.getAllData = function() {
		  $scope.loading = true;
		  var promise = DataLoad.getSharePageData({
		  ...
		  return promise.then(function (returnData) {
				if (returnData && returnData.code == 0) {
					...
					$scope.dataList = [];
					$scope.haveData = false;
					toaster.pop('error', '分享积分', returnData.message);
				}
				$scope.loading = false;
	...
	$scope.deleteBatch = function(){
	   if(confirm("确定要删除吗?") == true){
		  if($scope.submit == false){
			  ...
			  var promise = DataLoad.deleteShareBatch({
			  ...
			  return promise.then(function (returnData) {
					...
					if (returnData && returnData.code == 0) {
						toaster.pop('success', '分享积分', returnData.message);
						location.reload();
					} 
					else {
						$scope.submit = false;
