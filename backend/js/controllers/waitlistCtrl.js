...
.controller('WaitlistCtrl', ['$scope', 'CommonFunction', 'DataLoad', 'toaster', 'divpage', '$modal', '$stateParams', function($scope, CommonFunction, DataLoad, toaster, divpage, $modal, $stateParams) {
	$scope.haveData = true;
	$scope.loading = true;
	$scope.query = '';
	$scope.page = 1;
	...
	$scope.getAllData = function() {
		  $scope.loading = true;
		  var promise = DataLoad.getWaitPageData({
		  ...
		  return promise.then(function (returnData) {
				if (returnData && returnData.code == 0) {
					$scope.shoot_point = returnData.data.shoot_point;
					$scope.future_point = returnData.data.future_point;
					if (returnData.data.data.length != 0) {
						...
						angular.forEach($scope.dataList, function(value, key) {
							value.clear_html = '结算';
							value.clear_disabled = false;
					    });
					} else {
						$scope.dataList = [];
						toaster.pop('success', '待用积分', "没有数据！");
					...
					$scope.dataList = [];
					$scope.haveData = false;
					toaster.pop('error', '待用积分', returnData.message);
				}
				$scope.loading = false;
	...
	$scope.deleteBatch = function(){
	   if(confirm("确定要删除吗?") == true){
		  if($scope.submit == false){
			  ...
			  var promise = DataLoad.deleteWaitBatch({
			  ...
			  return promise.then(function (returnData) {
					...
					if (returnData && returnData.code == 0) {
						toaster.pop('success', '待用积分', returnData.message);
						location.reload();
					} 
					else {
						$scope.submit = false;
	...
	$scope.todo = function(item){
	   if(confirm("确定要结算吗?") == true){
		  if($scope.submit == false){
			  $scope.submit = true;
			  item.clear_html = '提交中…';
			  item.clear_disabled = true;
			  
			  var promise = DataLoad.clearWaitPoint({
				  id: item.id,
			  });
			  return promise.then(function (returnData) {
					if (returnData && returnData.code == 0) {
