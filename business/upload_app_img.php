...
if ( !empty( $_FILES ) ) {
    ...
    if ($_FILES['file']["error"] > 0){
		...
		$uploadPath = dirname( __FILE__ ) . DIRECTORY_SEPARATOR . 'app_uploads' . DIRECTORY_SEPARATOR . $file;
    	...
    	$newname = time().substr($fileButtonName, 6);
    	$file = $newname.'.'.$ext;
    	$uploadPath = dirname( __FILE__ ) . DIRECTORY_SEPARATOR . 'app_uploads' . DIRECTORY_SEPARATOR . $file;
    	...
    	if (file_exists($uploadPath)){
		    ...
		    $answer = array( 'code' => 0,  'answer' => '文件上传成功 !', 'filename'=>$file );
    	}
    }
