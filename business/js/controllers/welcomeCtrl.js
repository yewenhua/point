...
angular.module('app').controller('WelcomeCtrl', ['$scope', 'DataLoad', '$window', function($scope, DataLoad, $window) {
    ...
    $scope.goLink = function(param) {
    	...
    	if(param == 'send'){
    	    ...
    	    $window.location.href = "/merchant#/app/orderlist?status=2&start=" + start + "&end=" + end;
    	}
    	else if(param == 'complete'){
    	    $window.location.href = "/merchant#/app/orderlist?status=3&start=" + start + "&end=" + end;
    	}
    	else if(param == 'refund'){
    	    $window.location.href = "/merchant#/app/orderlist?status=7&start=" + start + "&end=" + end;
    	}
    }
