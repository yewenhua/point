...
.controller('OrderlistCtrl', ['$scope', 'CommonFunction', 'DataLoad', 'toaster', 'divpage', '$modal', '$stateParams', '$document', function($scope, CommonFunction, DataLoad, toaster, divpage, $modal, $stateParams, $document) {
	$scope.haveData = true;
	$scope.loading = true;
	$scope.query = '';
	$scope.page = 1;
	...
	$scope.tableExport = function(){
		  angular.element($document[0].querySelector('#tableForm')).submit();
	}
