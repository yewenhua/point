      ...
      $rootScope.getSystemData = function() {
		  ...
		  return promise.then(function (returnData) {
				if (returnData && returnData.code == 0) {
					...
					$rootScope.systemInfo.signin_title = $rootScope.systemInfo.title + '商家管理后台';
					$scope.app.name = angular.copy($rootScope.systemInfo.title);
				} 
				else {
					$rootScope.systemInfo = {};
					$rootScope.systemInfo.title = '微商商家管理系统';
					$rootScope.systemInfo.signin_title = '微商商家后台管理系统';
    ...
    /**
      ...
      */
	  ...
	  $rootScope.getNewMessage = function() {
		  ...
		  return promise.then(function (returnData) {
			  if (returnData && returnData.code == 0) {
				  if(returnData.count > 0 && returnData.count != $rootScope.diffNum_no_review){
					  $rootScope.alert_no_review_title = '待发货';
					  $rootScope.diffNum_no_review = angular.copy(returnData.count);
      ...
      $scope.closeReviewMessage = function($event) {
    	  ...
    	  $("#closeReviewMessage").fadeOut();
    	  $window.location.href = "/merchant#/app/orderlist?status=1&start=" + start + "&end=" + end;
      }
