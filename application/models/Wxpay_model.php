...
class Wxpay_model extends CI_Model {
	...
	public function __construct() {
		parent::__construct();
		$this->notify_url = base_url("orders/notify");
	}
    ...
    public function transfers($openid, $true_name, $amount, $desc){
        ...
        $this->companyPayParameters['mch_appid']  = $this->config->item('appId');
        $this->companyPayParameters['mchid']     = $this->config->item('Mchid');
    ...
    public function getTransferInfo($partner_trade_no){
        //最终返回值
        $this->companyPayParameters = array();
        $this->companyPayParameters['appid']  = $this->config->item('appId');
        $this->companyPayParameters['mch_id']     = $this->config->item('Mchid');
        $this->companyPayParameters['nonce_str']  = $this->createNoncestr();
	...
	public function prepay($order_id, $openid, $type, $fee) {
		...
		$this->setPayParameters('appid', $this->config->item('appId'));//公众账号ID
	   	$this->setPayParameters('mch_id', $this->config->item('Mchid'));//商户号
		...
		$this->setPayParameters('body', '商品支付'); //商品描述 
	...
	public function orderquery($transaction_id, $out_trade_no) {
		...
		$this->result = $this->xmlToArray($this->response);
		log_message('info', $out_trade_no.'查询订单返回参数：'.var_export($this->result, true));
    ...
    public function refund($transaction_id, $out_trade_no, $total_fee, $refund_fee) {
		...
		$this->setPayParameters('op_user_id', $this->config->item('Mchid'));  //操作员帐号, 默认为商户号
		$this->publicPayParameters["sign"] = $this->getSign($this->publicPayParameters);//签名
		$xml = $this->arrayToXml($this->publicPayParameters);
	...
	public function createQrcodeUrl($product_id) {
		...
		$this->setScanParameters('appid', $this->config->item('appId'));
		$this->setScanParameters('mch_id', $this->config->item('Mchid'));
	...
	public function native_call() {
	    ...
	    if($this->checkSign($callbackData) != FALSE){
		    ...
		    if($rtn != null){
				...
				$this->setReturnParameters('appid', $this->config->item('appId'));
			    $this->setReturnParameters('mch_id', $this->config->item('Mchid'));
	...
	{
		$jsApiObj = array();
		$jsApiObj["appId"] = $this->config->item('appId');
		$timeStamp = time();
   ...
   /**
	...
	private function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$this->config->item('WxPayKey');
		//签名步骤三：MD5加密
		$String = md5($String);
		//签名步骤四：所有字符转为大写
	...
	private function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, $this->config->item('SSLCERT_PATH'));
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, $this->config->item('SSLKEY_PATH'));
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
