<?php
class Tencent_model extends CI_Model {
	...
	public function __construct() {
		parent::__construct();
		$this->appId = $this->config->item('serviceAppId');
        $this->appSecret = $this->config->item('serviceAppSecret');
	}
	...
	*/
	public function getAccessToken() {
	    $data = json_decode(file_get_contents(dirname(__FILE__)."/service_access_token.json"));
        if ($data == null || empty($data) || $data->expire_time < time()) {
			...
			if($result && isset($result->access_token)){
                ...
                $fp = fopen(dirname(__FILE__)."/service_access_token.json", "w");
				...
				$accessToken = null;
				$fileName = dirname(__FILE__)."/service_access_token.json";
	...
	*/
    public function createMenu() {
    	...
    	if($access_token != null){
    	    $menu_body = $this->config->item('service_menu_array');
	        //https请求
  ...
  /**
	...
	*/
	public function getJsapiTicket() {
	    $data = json_decode(file_get_contents(dirname(__FILE__)."/service_jsapi_ticket.json"));
        if ($data == null || empty($data) || $data->expire_time < time()) {
	        if ($data == null || empty($data)){
	        	log_message('info', 'service_jsapi_ticket为空');
	    		$data = (object)array();
	    	}
			...
			if($accesstoken !== null){
				...
				if($result && $result->errcode == 0 &&  isset($result->ticket)){
	                ...
	                $fp = fopen(dirname(__FILE__)."/service_jsapi_ticket.json", "w");
	                fwrite($fp, json_encode($data));
	                fclose($fp);
				}
				else{
					...
					$jsapi_ticket = null;
					$fileName = dirname(__FILE__)."/service_jsapi_ticket.json";
					if(file_exists($fileName)){
	...
	public function curl_post($url, $data){ // 模拟提交数据函数
	    ...
	    if (curl_errno($curl)) {
		    ...
		    if($result && isset($result->errcode) && $result->errcode == 40001){
		        $fileName = dirname(dirname(dirname(__FILE__)))."/application/models/service_access_token.json";
				if(file_exists($fileName)){
