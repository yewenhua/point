<?php
class Qqpay_model extends CI_Model {
	...
	private $serviceMchid;
	
	public function __construct() {
		parent::__construct();
		$this->notify_url = base_url("service/online_notify");
		$this->serviceAppId = $this->config->item('serviceAppId');
		$this->serviceMchid = $this->config->item('serviceMchid');
	}
    ...
    public function transfers($openid, $true_name, $amount, $desc){
        ...
        $this->companyPayParameters['mch_appid']  = $this->serviceAppId;
        $this->companyPayParameters['mchid']     = $this->serviceMchid;
        ...
        log_message('info', 'transfers param:'.var_export($xmlData, true));
    ...
    public function getTransferInfo($partner_trade_no){
        //最终返回值
        $this->companyPayParameters = array();
        $this->companyPayParameters['appid']  = $this->serviceAppId;
        $this->companyPayParameters['mch_id']     = $this->serviceMchid;
        $this->companyPayParameters['nonce_str']  = $this->createNoncestr();
        ...
        //准备XML数据库和curl相关参数
        $xmlData = $this->arrayToXml($this->companyPayParameters);
        log_message('info', 'getTransferInfo param:'.var_export($xmlData, true));
	 ...
	 */
	public function prepay($order_id, $openid, $type, $fee) {
		$this->publicPayParameters = array();
		$this->setPayParameters('appid', $this->serviceAppId);//公众账号ID
	   	$this->setPayParameters('mch_id', $this->serviceMchid);//商户号
		...
		$this->setPayParameters('body', '报单充值'); //商品描述 
		$this->setPayParameters('out_trade_no', $order_id);
		...
		$this->response = $this->postXmlCurl($xml, $this->prepay_url, $this->curl_timeout);
		$this->result = $this->xmlToArray($this->response);
		
		$log_data = $this->result;
		$log_data['order_id'] = $order_id;
		$log_data['openid'] = $openid;
		$log_data['fee'] = $fee;
		log_message('info', '服务中心预支付订单返回结果：'.var_export($log_data, true));
	...
	public function notify() {
		...
		log_message('info', '支付回调参数：'.var_export($callbackData, true));
		$status = '';
	...
	public function orderquery($transaction_id, $out_trade_no) {
		...
		$this->result = $this->xmlToArray($this->response);
		log_message('info', $out_trade_no.'服务中心查询订单返回参数：'.var_export($this->result, true));
    ...
    public function closeorder($out_trade_no) {
		...
		$this->result = $this->xmlToArray($this->response);
		log_message('info', '关闭订单返回参数：'.var_export($this->result, true));
    ...
    public function refund($transaction_id, $out_trade_no, $total_fee, $refund_fee) {
		...
		$this->setPayParameters('op_user_id', $this->serviceMchid);  //操作员帐号, 默认为商户号
		$this->publicPayParameters["sign"] = $this->getSign($this->publicPayParameters);//签名
		$xml = $this->arrayToXml($this->publicPayParameters);
		$this->response = $this->postXmlCurl($xml, $this->refund_url, $this->curl_timeout);
		$this->result = $this->xmlToArray($this->response);
		log_message('info', '退款申请返回参数：'.var_export($this->result, true));
    ...
    public function refundquery($transaction_id, $out_trade_no, $out_refund_no, $refund_id) {
		...
		$this->result = $this->xmlToArray($this->response);
		log_message('info', '退款查询返回参数：'.var_export($this->result, true));
	...
	public function createQrcodeUrl($product_id) {
		...
		$this->setScanParameters('appid', $this->serviceAppId);
		$this->setScanParameters('mch_id', $this->serviceMchid);
	...
	public function native_call() {
	    ...
	    if($this->checkSign($callbackData) != FALSE){
		    ...
		    if($rtn != null){
				...
				$this->setReturnParameters('appid', $this->serviceAppId);
			    $this->setReturnParameters('mch_id', $this->serviceMchid);
	...
	{
		$jsApiObj = array();
		$jsApiObj["appId"] = $this->serviceAppId;
		$timeStamp = time();
   ...
   /**
	...
	private function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$this->config->item('serviceWxPayKey');
		//签名步骤三：MD5加密
		$String = md5($String);
		//签名步骤四：所有字符转为大写
	...
	private function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, $this->config->item('SERVICE_SSLCERT_PATH'));
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, $this->config->item('SERVICE_SSLKEY_PATH'));
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
