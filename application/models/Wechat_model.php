...
class Wechat_model extends CI_Model {
	...
	public function getRecommendInfoByCode($code) {
	    $sql = "SELECT u.*,w.openid FROM user AS u INNER JOIN wechat AS w ON u.id=w.userid WHERE binary u.code = '".$code."'";
		$query = $this->db->query($sql);
	    if ($query->num_rows() >= 0){
	    	if($query->num_rows() == 0){
	    		return null;
...
PARAM;
	...
	*/
     ...
     * @param $memberInfo
     */
    public function createRegistPoster($memberInfo, $qrcodePath, $poster){
    	$personalDir = BASEPATH.'../wechat/userTgCache'.DIRECTORY_SEPARATOR.$memberInfo['id'];
    	$this->rmdirs($personalDir); 
    	$haibaoFilename= 'tg.png';
    	$returnFilePath = $personalDir.DIRECTORY_SEPARATOR.$haibaoFilename;
        if(!file_exists($returnFilePath)){
        	//生成的合成二维码并返回路径
        	$start_time = time();
            $bgImgPath = APPPATH.'..'.$poster['bg_url'];
            $json_data = json_decode($poster['json_data'], true);
            
            //背景画布
            $bg_context = stream_context_create($this->get_tg_stream_context_create_opts());
            $bg_str = file_get_contents($bgImgPath, false, $bg_context);
            ...
            //创建空画布
            $image_template = imagecreatetruecolor(320, 504);
			imagecopyresampled($image_template, $bg_img, 0, 0, 0, 0, 320, 504, $bg_width, $bg_height);
            
            $fontColor = imagecolorallocate($image_template, 0, 0, 0);;
            $fontSize = 14;
            ...
            //将真彩色图像转换为调色板图像
            //imagetruecolortopalette($image_template, false, 65535);

            foreach($json_data as $item){
            	if($item['type'] == 'head'){
            		//将头像取到画布中
            		$head_img_path = $memberInfo['headimgurl'];
            		$head_context = stream_context_create($this->get_tg_stream_context_create_opts());
            		$head_str = file_get_contents($head_img_path, false, $head_context);
            		
            		//从字符串中的图像流新建一图像
                    $head_img = imagecreatefromstring($head_str);
            		
            		/*
            		 * 将头像画布采样copy到真彩色调色板中，不会失真
            		 */
                    $dist_x = $item['left']; //载入的 图片在新图片中的x坐标
                    $dist_y = $item['top']; //载入的 图片在新图片中的y坐标
                    $src_x = 0; //设定载入图片要载入的区域x坐标
                    $src_y = 0; //设定载入图片要载入的区域y坐标
                    $dst_w = $item['width']; //设定载入的原图的宽度（在此设置缩放）
                    $dst_h = $item['height']; //设定载入的原图的高度（在此设置缩放）
                    $src_w = imagesx($head_img); //原图要载入的宽度
                    $src_h = imagesx($head_img); //原图要载入的高度
		            imagecopyresampled($image_template, $head_img, $dist_x, $dist_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h);
            	}
            	elseif($item['type'] == 'qr'){
            		//将二维码取到画布中
            		$qr_img_path = $qrcodePath;
            		$qr_context = stream_context_create($this->get_tg_stream_context_create_opts());
            		$qr_str = file_get_contents($qr_img_path, false, $qr_context);
            		
            		//从字符串中的图像流新建一图像
                    $qr_img = imagecreatefromstring($qr_str);
                    ...
                    $dst_w = $item['width']; //设定载入的原图的宽度（在此设置缩放）
                    $dst_h = $item['height']; //设定载入的原图的高度（在此设置缩放）
                    $src_w = imagesx($qr_img); //原图要载入的宽度
                    $src_h = imagesy($qr_img); //原图要载入的高度
		            imagecopyresampled($image_template, $qr_img, $dist_x, $dist_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h);
            	}
            	else{
            		$length = strlen($item['color']);
            		$start = strpos($item['color'], '(');
            		$end = strpos($item['color'], ')');
            		$rgb_str = substr($item['color'], $start + 1, -1);
            		$rgb_arr = explode(',', $rgb_str);

            		$fontColor = imagecolorallocate($image_template, intval($rgb_arr[0]), intval($rgb_arr[1]), intval($rgb_arr[2]));
            		$fontSize = intval(substr($item['fontSize'], 0, -2)) - 4;
            		$fontLeft = intval($item['left']);
            		$fontTop = intval($item['top']) + intval($item['height'])/2 + $fontSize/2;  //文字左下角的点的坐标为字体的坐标
            	}
            }

            //设定图像的混色模式并启用
            imagealphablending($image_template, true);
            
            //为一幅图像分配字体
            $font_file = BASEPATH.'../wechat/userTgCache/wryh.ttf';
            
            //使用 FreeType 2 字体将文本写入图像
            imagefttext($image_template, $fontSize, 0, $fontLeft, $fontTop, $fontColor, $font_file, $name);

            //建立 PNG 图型
            imagepng($image_template, $returnFilePath);
    ...
    public function isOpenidExistRegist($openid) {
        $sql = "SELECT * FROM member WHERE openid = '".$openid."' AND deleted_at is null";
		$query = $this->db->query($sql);
	    if ($query->num_rows() >= 0){
	    	if($query->num_rows() == 0){
	    		return null;
