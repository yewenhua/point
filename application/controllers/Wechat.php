...
define("TOKEN","08525b857f40f7b87ee4a0206e8e318f");
class Wechat extends CI_Controller {
                ...
                case 'event':
                    ...
                    elseif ($this->_USERDATA['event'] == 'click') {
                        ...
                        switch ($this->_USERDATA['event_key']) {
                            case 'V3001_HAIBAO': 
                            	$platformInfo = $this->wechat_model->isOpenidExist($openid);
	...
	*/
    ...
    public function subscribe($param){
        ...
        $userInfo = $this->weixin_model->getUserInfo($openid);
    ...
    public function unsubscribe($param){
        ...
        if($platformInfo !== null && !empty($platformInfo)){
        	$rst = $this->wechat_model->unsubscribe($openid);
        	if($result){
        		$message = '取消关注成功';
    ...
    public function createMenu() {
    	$this->load->model('weixin_model');
    	$result = $this->weixin_model->createMenu();
    	echo $result;
    }
	
	private function bindUser($param)
	{
		$platformInfo = $this->wechat_model->isOpenidExist($param['openid']);
		if($platformInfo === null){
			$is_subscribe = 0;
			$insertResult = $this->wechat_model->insertWechatInfo($param, $is_subscribe);
			...
			$updateResult = $this->wechat_model->updateAccesstoken($param);
   ...
   /**
    ...
    public function sendCustomMessageOfText($openid, $message){
        ...
        $rtn = $this->weixin_model->sendCustomMessage($postData);
        if($rtn){
	...
	*/
    ...
    public function sendHaibaoMsg() {
    	...
    	$platformInfo = $this->wechat_model->isOpenidExist($open_id);
    	if($platformInfo !== null){
    	    $media_path = $this->wechat_model->createHaibao($platformInfo);
	    	$type = 'image';
	    	$media_id = $this->wechat_model->uploadMediaFile($media_path, $type);
	    	if($media_id != null){
    ...
    private function updateUserInfo($userInfo, $recommendMember, $from, $type){
        //是否存在平台信息
        $platformInfo = $this->wechat_model->isOpenidExist($userInfo['openid']);
        if($platformInfo !== null){
            //存在则修改平台信息和修改用户信息
            if($type == 'self'){
                $message = "欢迎再次关注我们";
    		...
    		$rst = $this->wechat_model->updateWechatInfo($userInfo);
        }
        else{
            //不存在则插入平台信息和修改用户信息
            if($type == 'self'){
            ...
            $is_subscribe = 1;
    		$rst = $this->wechat_model->insertWechatInfo($userInfo, $is_subscribe);
        }
     ...
     * @param $radius
     */
    public function sendRegistPosterMsg() {
    	$msgData = array(
     	    'FromUserName'=>$this->input->post('FromUserName'),
     	);
    	$open_id = $msgData['FromUserName'];
    	$platformInfo = $this->wechat_model->isOpenidExistRegist($open_id);
    	if($platformInfo !== null){
    		$poster = $this->getPosterShare();
    		if($poster !== null){
	    		$registQrcodeImg = $this->generateRegistQrcode($platformInfo);
	    	    $media_path = $this->wechat_model->createRegistPoster($platformInfo, $registQrcodeImg, $poster);
		    	$type = 'image';
		    	$media_id = $this->wechat_model->uploadMediaFile($media_path, $type);
		    	if($media_id != null){
		    		$result = $this->sendCustomMessageOfImg($open_id, $media_id);
    			...
    			$result = array('code'=>10003, 'message'=>'海报未设置');
    		}
    	}
    	else{
    		$result = array('code'=>10004, 'message'=>'获取平台信息失败');
    ...
    private function generateRegistQrcode($memberInfo){
    	include_once dirname(__FILE__).'/phpqrcode.php';
    	$qr_filename = dirname(dirname(dirname(__FILE__))).'/media/qrcode/regist_'.$memberInfo['id'].'.png';
    	if(!file_exists($qr_filename)){
	    	$pwd_key = $this->config->item('pwd_key');
	    	$key = $this->encrypt($memberInfo['mobile'], $pwd_key);
			$url = 'http://www.ziyivip.com/member/regist?key='.$key;
    ...
    private function getPosterShare(){
    	$type = 1;
        $haibaoTemplate = $this->wechat_model->posterData($type);
        return $haibaoTemplate;
    }
    
    public function sendPosterNotWechat(){
    	$this->auth_json_member();
    	$userInfo = $this->user;
    	$openid = $userInfo['openid'];
    	$poster = $this->getPosterShare();
    	if($poster !== null){
    		$registQrcodeImg = $this->generateRegistQrcode($userInfo);
    	    $media_path = $this->wechat_model->createRegistPoster($userInfo, $registQrcodeImg, $poster);
	    	$type = 'image';
	    	
	    	$media_id = $this->wechat_model->uploadMediaFile($media_path, $type);
	    	if($media_id != null){
	    		$result = $this->sendCustomMessageOfImg($openid, $media_id);
	    	}
	    	else{
	    		$result = array('code'=>10002, 'message'=>'上传素材失败');
	    	}
    ...
    public function test(){
    	$openid = 'o6Hxfv8DxDCX7rEfCUibBU5qCQFY';
    	$platformInfo = $this->wechat_model->isOpenidExistRegist($openid);
    	if($platformInfo !== null){
    		$poster = $this->getPosterShare();
    		if($poster !== null){
	    		$registQrcodeImg = $this->generateRegistQrcode($platformInfo);
    		...
    		$result = array('code'=>10004, 'message'=>'获取个人信息失败');
    	}
    	echo json_encode($result);
    	exit;
    }
